...
export async function getChatResponse(message: string, history: { role: string, parts: { text: string }[] }[]) {
  const chat = ai.chats.create({
    ...
    config: {
      systemInstruction: "Você é o assistente de IA do A Fornalha, um restaurante de luxo em Chimoio, Moçambique. Você ajuda os clientes com informações sobre o menu, processo de pedido e consultas gerais. Seja educado, elegante e prestativo. Nossas especialidades são Pizzas Artesanais e Grelhados. Contacto: +258 86 767 4675.",
    },
  });
